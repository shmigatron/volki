...
fn format_no_files_clean_exit() {
    ...
    let output = volki().current_dir(&dir).args(["format", dir.to_str().unwrap()]).output().unwrap();
...
fn format_check_no_files_clean_exit() {
    ...
    let output = volki().current_dir(&dir).args(["format", dir.to_str().unwrap(), "--check"]).output().unwrap();
    assert!(output.status.success());
    let _ = fs::remove_dir_all(&dir);
}
...
fn format_writes_file() {
    ...
    assert!(result.contains("\"hello\""), "Expected double quotes, got: {result}");
    let _ = fs::remove_dir_all(&dir);
}
...
fn format_check_detects_changes() {
    ...
    let output = volki().current_dir(&dir).args(["format", dir.to_str().unwrap(), "--check"]).output().unwrap();
    assert!(!output.status.success(), "Check should fail when files need formatting");

    // File should NOT be modified in check mode
    let result = fs::read_to_string(&file).unwrap();
    assert!(result.contains("'hello'"), "Check mode should not modify files");
    let _ = fs::remove_dir_all(&dir);
}
...
fn format_check_passes_when_formatted() {
    ...
    fs::write(&file, "const x = 'hello'\n").unwrap();
    volki().current_dir(&dir).args(["format", dir.to_str().unwrap()]).output().unwrap();

    // Then check — should pass
    let output = volki().current_dir(&dir).args(["format", dir.to_str().unwrap(), "--check"]).output().unwrap();
    assert!(output.status.success(), "Check should pass on already-formatted files");
    let _ = fs::remove_dir_all(&dir);
}
...
fn format_inserts_semicolons() {
    ...
    let result = fs::read_to_string(&file).unwrap();
    assert!(result.contains("const x = 1;"), "Expected semicolons, got: {result}");
    assert!(result.contains("const y = 2;"), "Expected semicolons, got: {result}");
    let _ = fs::remove_dir_all(&dir);
}
...
fn format_normalizes_indentation() {
    ...
    let result = fs::read_to_string(&file).unwrap();
    assert!(result.contains("  foo();"), "Expected 2-space indent, got: {result}");
    let _ = fs::remove_dir_all(&dir);
}
...
fn format_single_file_path() {
    ...
    let output = volki().current_dir(&dir).args(["format", file.to_str().unwrap()]).output().unwrap();
    assert!(output.status.success());
...
fn format_skips_non_js_files() {
    ...
    assert_eq!(txt_content, "hello world", "Non-JS files should not be modified");
    let _ = fs::remove_dir_all(&dir);
}
