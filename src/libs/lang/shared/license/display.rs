...
use crate::core::volkiwithstds::io::Write;
...
fn print_header(w: &mut dyn Write, result: &ScanResult) {
    ...
    let _ = writeln!(
        ...
        style::dim(&crate::vformat!("({} packages)", result.total_packages)),
    );
    let _ = writeln!(w);
}
...
pub fn print_list(w: &mut dyn Write, result: &ScanResult) {
    ...
    for pkg in &result.packages {
        ...
        let padded = crate::vformat!("{:<width$}", pkg.license, width = max_license_len);
        let _ = writeln!(
            w,
            "  {}  {}",
            color_fn(&padded),
            style::dim(&crate::vformat!("{}@{}", pkg.name, pkg.version)),
        );
    }
    let _ = writeln!(w);
}
...
pub fn print_grouped(w: &mut dyn Write, result: &ScanResult) {
    ...
    for license in license_keys {
        ...
        let _ = writeln!(
            ...
            style::dim(&crate::vformat!("({} packages)", pkgs.len())),
        );
...
pub fn print_summary(w: &mut dyn Write, result: &ScanResult) {
    ...
    for (license, pkgs) in &entries {
        ...
        let bar: String = String::from("\u{2588}").repeat(bar_len);
        let padded_name = crate::vformat!("{:<width$}", license, width = max_name_len);
...
mod tests {
    ...
    fn make_result(project: &str, packages: Vec<PackageLicense>) -> ScanResult {
        ...
        ScanResult {
            project_name: project.to_vstring(),
    ...
    fn render(
        f: fn(&mut dyn crate::core::volkiwithstds::io::Write, &ScanResult),
        result: &ScanResult,
    ) -> String {
        let mut buf = crate::core::volkiwithstds::io::Cursor::new(
            crate::core::volkiwithstds::collections::Vec::new(),
        );
        f(&mut buf, result);
        let bytes = buf.into_inner();
        let s = core::str::from_utf8(bytes.as_slice()).unwrap();
        crate::core::volkiwithstds::collections::String::from(s)
    }
    ...
    fn list_empty_packages() {
        let result = make_result("test", vvec![]);
    ...
    fn list_single_package() {
        let result = make_result("myproject", vvec![make_pkg("lodash", "4.17.21", "MIT")]);
        let output = render(print_list, &result);
    ...
    fn list_header_shows_project_name() {
        let result = make_result("myproject", vvec![make_pkg("a", "1.0", "MIT")]);
        let output = render(print_list, &result);
    ...
    fn grouped_permissive_before_copyleft() {
        let result = make_result(
            "test",
            vvec![make_pkg("a", "1.0", "MIT"), make_pkg("b", "1.0", "GPL-3.0"),],
        );
        let output = render(print_grouped, &result);
    ...
    fn grouped_count_per_group() {
        let result = make_result(
            "test",
            vvec![make_pkg("a", "1.0", "MIT"), make_pkg("b", "2.0", "MIT"),],
        );
        let output = render(print_grouped, &result);
    ...
    fn summary_bar_chart_present() {
        let result = make_result("test", vvec![make_pkg("a", "1.0", "MIT")]);
        let output = render(print_summary, &result);
