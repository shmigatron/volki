            ...
            if let Some(main) = map.get("main").and_then(|v| v.as_str()) {
                ...
                if main_path.is_file() {
                    return Ok(ResolvedPlugin {
                        ...
                        version: map.get("version").and_then(|v| v.as_str()).map(String::from),
                    });
                }
            }
        }
...
fn read_node_version(pkg_dir: &Path) -> Option<String> {
    ...
    let map = extract_top_level(&content);
    map.get("version").and_then(|v| v.as_str()).map(String::from)
}
...
mod tests {
    ...
    fn resolve_node_volki_plugin_js() {
        ...
        fs::write(pkg.join("volki-plugin.js"), "// plugin").unwrap();

        let spec = PluginSpec { name: "my-plugin".into(), runtime: None, options: vec![] };
        let resolved = resolve(&spec, &dir).unwrap();
        assert_eq!(resolved.runtime, PluginRuntime::Node);
        assert!(resolved.entry_point.ends_with("volki-plugin.js"));
        cleanup(&dir);
    }
    ...
    fn resolve_node_package_json_main() {
        ...
        let pkg = dir.join("node_modules/my-plugin");
        fs::create_dir_all(&pkg).unwrap();
        fs::write(pkg.join("package.json"), r#"{"main":"lib/entry.js","version":"1.0.0"}"#).unwrap();
        fs::create_dir_all(pkg.join("lib")).unwrap();
        fs::write(pkg.join("lib/entry.js"), "// entry").unwrap();

        let spec = PluginSpec { name: "my-plugin".into(), runtime: None, options: vec![] };
        let resolved = resolve(&spec, &dir).unwrap();
        assert!(resolved.entry_point.ends_with("lib/entry.js"));
        assert_eq!(resolved.version, Some("1.0.0".into()));
        cleanup(&dir);
    }
    ...
    fn resolve_node_index_fallback() {
        ...
        let pkg = dir.join("node_modules/my-plugin");
        fs::create_dir_all(&pkg).unwrap();
        fs::write(pkg.join("index.js"), "// index").unwrap();

        let spec = PluginSpec { name: "my-plugin".into(), runtime: None, options: vec![] };
        let resolved = resolve(&spec, &dir).unwrap();
        assert!(resolved.entry_point.ends_with("index.js"));
        cleanup(&dir);
    }
    ...
    fn resolve_not_found() {
        ...
        let spec = PluginSpec { name: "nonexistent".into(), runtime: None, options: vec![] };
        assert!(matches!(resolve(&spec, &dir), Err(PluginError::NotFound(_))));
        cleanup(&dir);
    }
    ...
    fn resolve_python_plugin() {
        ...
        fs::write(site.join("volki_plugin.py"), "# plugin").unwrap();

        let spec = PluginSpec { name: "my-plugin".into(), runtime: None, options: vec![] };
        let resolved = resolve(&spec, &dir).unwrap();
        assert_eq!(resolved.runtime, PluginRuntime::Python);
        assert!(resolved.entry_point.ends_with("volki_plugin.py"));
        cleanup(&dir);
    }
}
